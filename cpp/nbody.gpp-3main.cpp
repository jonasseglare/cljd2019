...
/* The Computer Language Benchmarks Game
   ...
   Modified by Jonas Östlund to make use of the benjmark library.
*/
...
struct Setup {
  Problem input(const nlohmann::json& src) const {
    ...
    dst.step_size = src["step-size"];
    return dst;
  }
  
  Output compute(const Problem& problem) const {
    NBodySystem system;
    Output output;
    for (int i = 0; i < problem.iterations; i++) {
      system.advance(problem.step_size);
    }
    output.energy = system.energy();
    return output;
  }

  nlohmann::json output(const Output& out) const {
    nlohmann::json dst;
    dst["energy"] = out.energy;
    return dst;
  }
};
